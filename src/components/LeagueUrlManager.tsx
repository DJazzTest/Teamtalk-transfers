...
export const LeagueUrlManager: React.FC<LeagueUrlManagerProps> = () => {
  ...
  useEffect(() => {
    ...
    if (savedUrls) {
      ...
      setUrls(defaultUrls);
      localStorage.setItem(getStorageKey(), JSON.stringify(defaultUrls));
    }

    // Listen for crawl status updates
  ...
  const addUrl = () => {
    ...
    setUrls(updatedUrls);
    localStorage.setItem(getStorageKey(), JSON.stringify(updatedUrls));
    setNewUrl('');
    toast({
      title: "URL Added",
      ...
      <div className="p-6">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            <p className="text-sm text-blue-200">{leagueName} Sources ({urls.length})</p>
            {urls.length === 0 ? (
