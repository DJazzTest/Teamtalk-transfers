/**
 * Chelsea Player Data Scraper from SofaScore
 * Usage: node scripts/scrapeChelseaPlayersSofaScore.mjs
 * Writes ./chelsea-players-data.json
 */
...
async function findPlayerId(playerName, clubName = 'Chelsea') {
  try {
    ...
    if (data?.players?.length) {
      ...
      for (const p of data.players) {
        ...
        if (team.includes('chelsea')) return p.id;
      }
...
async function main() {
  console.log('🚀 Chelsea – SofaScore scrape\n');
  const squadPath = path.join(__dirname, '..', 'src', 'data', 'squadWages.ts');
  const raw = fs.readFileSync(squadPath, 'utf-8');
  const m = raw.match(/'Chelsea':\s*createImageOnlySquad\s*\(\s*'chelsea',\s*\[([\s\S]*?)\]\s*\)/);
  if (!m) { console.error('Chelsea squad not found in squadWages.ts'); return; }
  ...
  const outPath = path.join(__dirname, '..', 'chelsea-players-data.json');
