/**
 * Manchester United Player Data Scraper from SofaScore
 * Usage: node scripts/scrapeManUnitedPlayersSofaScore.mjs
 * Writes ./man-utd-players-data.json
 */
...
async function findPlayerId(playerName, clubName = 'Manchester United') {
  try {
    ...
    if (data?.players?.length) {
      for (const p of data.players) {
        ...
        if (team.includes('man united') || team.includes('manchester united') || team.includes('man utd')) return p.id;
      }
...
async function main() {
  console.log('🚀 Manchester United – SofaScore scrape\n');
  const squadPath = path.join(__dirname, '..', 'src', 'data', 'squadWages.ts');
  const raw = fs.readFileSync(squadPath, 'utf-8');
  const m = raw.match(/'Manchester United':\s*createImageOnlySquad\s*\(\s*'man-utd',\s*\[([\s\S]*?)\]\s*\)/);
  if (!m) { console.error('Manchester United squad not found in squadWages.ts'); return; }
  ...
  const outPath = path.join(__dirname, '..', 'man-utd-players-data.json');
