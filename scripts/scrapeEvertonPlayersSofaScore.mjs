/**
 * Everton Player Data Scraper from SofaScore
 * Usage: node scripts/scrapeEvertonPlayersSofaScore.mjs
 * Writes ./everton-players-data.json
 */
...
async function findPlayerId(playerName, clubName = 'Everton') {
  try {
    ...
    if (data?.players?.length) {
      for (const p of data.players) {
        ...
        if (team.includes('everton')) return p.id;
      }
...
async function main() {
  console.log('🚀 Everton – SofaScore scrape\n');
  const squadPath = path.join(__dirname, '..', 'src', 'data', 'squadWages.ts');
  const raw = fs.readFileSync(squadPath, 'utf-8');
  const m = raw.match(/'Everton':\s*createImageOnlySquad\s*\(\s*'everton',\s*\[([\s\S]*?)\]\s*\)/);
  if (!m) { console.error('Everton squad not found in squadWages.ts'); return; }
  ...
  const outPath = path.join(__dirname, '..', 'everton-players-data.json');
