/**
 * Bournemouth Player Data Scraper from SofaScore
 * Usage: node scripts/scrapeBournemouthPlayersSofaScore.mjs
 * Writes ./bournemouth-players-data.json
 */
...
async function findPlayerId(playerName, clubName = 'Bournemouth') {
  try {
    ...
    if (data?.players?.length) {
      for (const p of data.players) {
        ...
        if (team.includes('bournemouth')) return p.id;
      }
...
async function main() {
  console.log('🚀 Bournemouth – SofaScore scrape\n');
  const squadPath = path.join(__dirname, '..', 'src', 'data', 'squadWages.ts');
  const raw = fs.readFileSync(squadPath, 'utf-8');
  const m = raw.match(/'Bournemouth':\s*createImageOnlySquad\s*\(\s*'bournemouth',\s*\[([\s\S]*?)\]\s*\)/);
  if (!m) { console.error('Bournemouth squad not found in squadWages.ts'); return; }
  ...
  const outPath = path.join(__dirname, '..', 'bournemouth-players-data.json');
