/**
 * Fulham Player Data Scraper from SofaScore
 * Usage: node scripts/scrapeFulhamPlayersSofaScore.mjs
 * Writes ./fulham-players-data.json
 */
...
async function findPlayerId(playerName, clubName = 'Fulham') {
  try {
    ...
    if (data?.players?.length) {
      for (const p of data.players) {
        ...
        if (team.includes('fulham')) return p.id;
      }
...
async function main() {
  console.log('🚀 Fulham – SofaScore scrape\n');
  const squadPath = path.join(__dirname, '..', 'src', 'data', 'squadWages.ts');
  const raw = fs.readFileSync(squadPath, 'utf-8');
  const m = raw.match(/'Fulham':\s*createImageOnlySquad\s*\(\s*'fulham',\s*\[([\s\S]*?)\]\s*\)/);
  if (!m) { console.error('Fulham squad not found in squadWages.ts'); return; }
  ...
  const outPath = path.join(__dirname, '..', 'fulham-players-data.json');
