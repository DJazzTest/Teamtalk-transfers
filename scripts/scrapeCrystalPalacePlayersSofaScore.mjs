/**
 * Crystal Palace Player Data Scraper from SofaScore
 * Usage: node scripts/scrapeCrystalPalacePlayersSofaScore.mjs
 * Writes ./crystal-palace-players-data.json
 */
...
async function findPlayerId(playerName, clubName = 'Crystal Palace') {
  try {
    ...
    if (data?.players?.length) {
      for (const p of data.players) {
        ...
        if (team.includes('crystal palace') || team.includes('palace')) return p.id;
      }
...
async function main() {
  console.log('🚀 Crystal Palace – SofaScore scrape\n');
  const squadPath = path.join(__dirname, '..', 'src', 'data', 'squadWages.ts');
  const raw = fs.readFileSync(squadPath, 'utf-8');
  const m = raw.match(/'Crystal Palace':\s*createImageOnlySquad\s*\(\s*'crystal-palace',\s*\[([\s\S]*?)\]\s*\)/);
  if (!m) { console.error('Crystal Palace squad not found in squadWages.ts'); return; }
  ...
  const outPath = path.join(__dirname, '..', 'crystal-palace-players-data.json');
