/**
 * Liverpool Player Data Scraper from SofaScore
 * Usage: node scripts/scrapeLiverpoolPlayersSofaScore.mjs
 * Writes ./liverpool-players-data.json
 */
...
async function findPlayerId(playerName, clubName = 'Liverpool') {
  try {
    ...
    if (data?.players?.length) {
      for (const p of data.players) {
        ...
        if (team.includes('liverpool')) return p.id;
      }
...
async function main() {
  console.log('🚀 Liverpool – SofaScore scrape\n');
  const squadPath = path.join(__dirname, '..', 'src', 'data', 'squadWages.ts');
  const raw = fs.readFileSync(squadPath, 'utf-8');
  const m = raw.match(/'Liverpool':\s*createImageOnlySquad\s*\(\s*'liverpool',\s*\[([\s\S]*?)\]\s*\)/);
  if (!m) { console.error('Liverpool squad not found in squadWages.ts'); return; }
  ...
  const outPath = path.join(__dirname, '..', 'liverpool-players-data.json');
