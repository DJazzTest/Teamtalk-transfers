/**
 * Burnley Player Data Scraper from SofaScore
 * Usage: node scripts/scrapeBurnleyPlayersSofaScore.mjs
 * Writes ./burnley-players-data.json
 */
...
async function findPlayerId(playerName, clubName = 'Burnley') {
  try {
    ...
    if (data?.players?.length) {
      for (const p of data.players) {
        ...
        if (team.includes('burnley')) return p.id;
      }
...
async function main() {
  console.log('🚀 Burnley – SofaScore scrape\n');
  const squadPath = path.join(__dirname, '..', 'src', 'data', 'squadWages.ts');
  const raw = fs.readFileSync(squadPath, 'utf-8');
  const m = raw.match(/'Burnley':\s*createImageOnlySquad\s*\(\s*'burnley',\s*\[([\s\S]*?)\]\s*\)/);
  if (!m) { console.error('Burnley squad not found in squadWages.ts'); return; }
  ...
  const outPath = path.join(__dirname, '..', 'burnley-players-data.json');
