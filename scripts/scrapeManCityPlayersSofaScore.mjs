/**
 * Manchester City Player Data Scraper from SofaScore
 * Usage: node scripts/scrapeManCityPlayersSofaScore.mjs
 * Writes ./man-city-players-data.json
 */
...
async function findPlayerId(playerName, clubName = 'Manchester City') {
  try {
    ...
    if (data?.players?.length) {
      for (const p of data.players) {
        ...
        if (team.includes('man city') || team.includes('manchester city')) return p.id;
      }
...
async function main() {
  console.log('🚀 Manchester City – SofaScore scrape\n');
  const squadPath = path.join(__dirname, '..', 'src', 'data', 'squadWages.ts');
  const raw = fs.readFileSync(squadPath, 'utf-8');
  const m = raw.match(/'Manchester City':\s*createImageOnlySquad\s*\(\s*'man-city',\s*\[([\s\S]*?)\]\s*\)/);
  if (!m) { console.error('Manchester City squad not found in squadWages.ts'); return; }
  ...
  const outPath = path.join(__dirname, '..', 'man-city-players-data.json');
